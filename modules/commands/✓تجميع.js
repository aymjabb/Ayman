module.exports.config = {
  ...
  hasPermssion: 0,
  credits: "عبدالرحمن",
  description: "لعبة تجميع الكلمات بطابع سيرا 🐱💥 مع مؤقت وزخارف",
  usages: ["لعبة"],
...
const questions = [
  ...
  { question: "ا ن ف ص ل", answer: "انفصل" },
...
module.exports.handleReply = async function ({ api, event, handleReply, Currencies, Users }) {
  ...
  if (now > handleReply.endTime) {
    api.sendMessage(`⏰ الوقت انتهى يا ${userName}! حاول أسرع المرة القادمة 😹`, event.threadID);
    return api.unsendMessage(handleReply.messageID);
  }

  if (userAnswer === correctAnswer) {
    Currencies.increaseMoney(event.senderID, 50);
    api.sendMessage(`🎉😻 واااو! ${userName} تجميعك صحيح!💥\n💰 ربحك: 50 دولار 🤑\nسيرا تقول: "يا سلام! 👏✨"`, event.threadID);
    api.unsendMessage(handleReply.messageID); 
  } else {
    api.sendMessage(`😹 خطأ يا ${userName}! حاول مرة أخرى بسرعة! ⏳`, event.threadID);
  }
};
...
module.exports.run = async function ({ api, event }) {
  ...
  const message = `🔥🎯 سيرا تقول: "من الأسرع في تجميع الكلمة؟ 😸"\n💡 الكلمة المفككة: ${question}\n⏱️ لديك 15 ثانية فقط!\n💰 أسرع شخص يفوز 50 دولار\n✨🎉🕹️😻`;
